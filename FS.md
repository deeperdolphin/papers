...
where $q\in2\mathbb{N}$ (so that $H$ is Hermitian and even, and conserves fermion parity), $E$ is the hyperedge set of a $q$-uniform hypergraph on the $M$ modes, and the $J_S$ are independent Gaussian couplings. Two sparsity parameters are used, following the distinction that the sparse-SYK literature makes:
...
**Canonical graph for diagnostics.** $\mathcal{S}_F^{(\eta)}$ is a hypergraph, and spectral gap, conductance, diameter, and ball growth are not unique for hypergraphs: different hypergraph random walks define different Laplacians [27]. All four diagnostics are therefore computed on one declared representation, the **weighted incidence (star-expansion) bipartite graph** $\mathcal{B}_F$: one node per site, one node per hyperedge of $\mathcal{S}_F^{(\eta)}$, and an incidence edge between site $i$ and hyperedge $e\ni i$ of weight $\omega(w_e,|e|)$ for a declared function $\omega$ (default $w_e^2/|e|$). Site-to-site distances are half the bipartite distances. Null ensembles are constructed on the same representation and matched in site-degree sequence, hyperedge-size distribution, and weight distribution — not merely in ordinary degree — because after an interacting $V$ supports have varying cardinality and an ensemble matched only in site degree would not control for that.
...
- **A pre-test fails:** the state cost does not vary across candidates (spectrum selection), or no spectral window exists; or **no $F$ compresses** within exhaustible $\mathcal{F}_0$ at $M\le14$ and sampled or ansatz searches at $M\le24$ fail systematically. The small-$M$ conjecture is falsified for $\mathcal{C}_{\mathrm{cert}}$ within $\mathcal{F}_0$ at $M\le14$ (the restricted certification test) and disfavored for (1.1) beyond it for the class (2.1).
...
[27] K. Hayashi, S. G. Aksoy, C. H. Park, H. Park, "Hypergraph random walks, Laplacians, and clustering," *Proc. CIKM* (2020), arXiv:2006.16377.
