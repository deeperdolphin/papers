...
The principle is static, so it does not consume the clock it might later produce; it is not vacuous, because with unrestricted transformations every state is a product state in some basis and with the state term removed the selection is fixed by the spectrum alone. The paper's single conjecture is that for expander couplings and ground-space states a factorization exists that is substantially cheaper than the ultraviolet presentation, whose support hypergraph has polynomial rather than exponential ball growth, and which changes reproducibly when the state changes at fixed $H$. We give a nested search over transformation classes designed so that the conjecture can be falsified within a declared class rather than merely not found, and a spectral pre-test framed as an empirical question. No numerics are reported, no gravitational, cosmological, or particle-physics claims are made, and Section 7 lists what earlier versions claimed and this one does not. Predecessors, in particular the loss-functional co-emergence of locality of Shokrian Zini, Brown, and Freedman, are compared in Section 8.
...
It is a definition of a class of ultraviolet Hamiltonians, a definition of a class of factorizations, a selection principle, one conjecture, and an experiment designed so that the conjecture can fail in a declared sense. Its one interesting prediction is that a non-geometric $H$ and a suitable $\Psi$ can jointly select a finite-dimensional local tensor-product structure, and, more sharply, that $F^*(H,\Psi_1)\neq F^*(H,\Psi_2)$ at fixed $H$. It does not derive gravity, Lorentz invariance, the dimension of space, the cosmological constant, or matter content, and it makes no claim about neutrinos, black-hole spectra, or intermediate scales. Earlier versions did; Section 7 records the withdrawals. Appendix E records the broader picture that motivated the proposal, labelled as bets that do not follow from anything in Sections 2–6.
...
A random $r$-regular $q$-uniform hypergraph is, with high probability, an expander: balls in its natural metric grow exponentially and every cut through a ball is proportional to the ball's volume. Consequently the factorization in which the sites are the ultraviolet modes themselves has a support hypergraph (Section 3.3) equal to $E$ and is not geometric. Any geometric minimizer of (1.1) is therefore a different object from the coupling list; nothing is planted.
...
A state is **physical**, for the purposes of the conjecture of Section 4.6, if it lies in the ground space of $H$; when the spectrum is gapped, the first excited state is included. Complexity relative to a reference plays no role in this definition, so the vacuity of Appendix B cannot enter it.

Three classes of **control** state are used in the experiment, each with an expected outcome:
...
- *Thermal states* at a temperature above the low-energy scale: volume-law in every legal factorization, hence with $L(\Psi\mid F)$ extensive everywhere; expected to reduce the minimizer to the cheapest writing of $H$, the spectrum-selection limit of Section 4.5.
...
- $O\in\mathcal{G}_s$, a rotation of the modes, $\chi'=O\chi$, drawn from a **declared finite class**: products of at most $s$ Givens rotations on pairs of modes, with angles from a finite alphabet of $2^{p}$ values. $\mathcal{G}_s$ is nested in $s$, $\mathcal{G}_0=\{\mathbb{1}\}$, and for small $s$ is exhaustively enumerable. General $O\in O(M)$ at precision $p$ is the limit $s\to\binom{M}{2}$ and costs $\sim\binom{M}{2}p$ bits (Section 4.1).
- $\{B_i\}$, a partition of the rotated modes into $n=M/2m$ sites of $2m$ modes each; $m$ is a declared convention, scanned.
- $V\in\mathcal{V}_D$, a parity-preserving unitary from a declared class of circuits of depth at most $D=O(1)$ built from even gates on $O(1)$ rotated modes.
...
Expanding $VOHO^{\mathsf T}V^\dagger$ in products of rotated modes and grouping by site, each term touches a set of sites. The **support hypergraph** $\mathcal{S}_F$ has the sites as vertices and every touched set as a hyperedge, weighted by the summed coupling magnitude on it. It is exact and is not chosen.

The **codebook** $G_F$ is a hypergraph on the same sites chosen as part of the description of $H$: terms whose support is a codebook hyperedge are listed cheaply, others expensively. An optimizer left free to choose $G_F$ would pick a lattice as a dictionary and hide the couplings in the residual; the cost of listing $G_F$ and the honesty cut of Section 5.4 (most coupling weight of $\mathcal{S}_F$ on codebook hyperedges) prevent this. Geometric diagnostics are computed on $\mathcal{S}_F$ restricted to its weight-carrying hyperedges, never on the codebook.
...
- $L(F)$: bits to specify the transformation. For $O\in\mathcal{G}_s$: $s$ pair labels at $\log_2\binom{M}{2}$ bits each and $s$ angles at $p$ bits, plus $\log_2 s$. For the partition: $\log_2$ of the number of partitions of $M$ modes into blocks of $2m$. For $V\in\mathcal{V}_D$: the gate list at declared cost per gate. A general rotation at precision $p$ would cost $\sim\binom{M}{2}p$ bits.
- $L(H\mid F)$: bits to list a codebook $G_F$ as an adjacency structure, then each term of $VOHO^{\mathsf T}V^\dagger$ as a codebook hyperedge index (or, off-codebook, an arbitrary site set at $q\log_2n$ bits), an operator label within the site tensor product on that support, and a coefficient at precision $p$.
...
The **ultraviolet presentation** is $F_0=(\mathbb{1},\{B_i\}_{\mathrm{UV}},\mathbb{1})$ with the codebook $E$: $L(F_0)$ is the partition cost alone, $L(H\mid F_0)=L(E)+|E|(\log_2|E|+O(q)+p)$, and $L(\Psi\mid F_0)$ is whatever the physical state costs on the expander. The quantity the experiment measures is the compression $\Delta\mathcal{C}=\mathcal{C}(F_0)-\mathcal{C}(F^*)$.

### 4.2 Why the model cost matters, and what it implies

Charging for $F$ has a consequence worth stating before any computation. For the physical states of Section 2.4 the state term can be large: a ground state of a sparse SYK Hamiltonian is highly entangled across mode bipartitions, so $L(\Psi\mid F_0)$ may approach the exponential cost of a generic state, and a rotation that made it area-law on some $G_F$ would save exponentially many bits, easily paying $\binom{M}{2}p$ for a general rotation. But the same rotation acts on $H$: a $q$-body term in the undressed modes becomes a sum of up to $M^q$ $q$-body terms in the rotated modes, so a generic $O$ makes $H$ dense and its support hypergraph complete rather than geometric. The principle is therefore a real three-way tension — the transformation must be cheap, the Hamiltonian must stay sparse and few-body on a codebook, and the state must become compressible on that codebook — and none of the three can be bought with the others for free. A geometric $F^*$ exists only if the compression of $H$ and $\Psi$ together pays for the change of variables that reveals it. This is the sharpened content of the proposal, and it is what an uncharged $O$ would have concealed.
...
The Hamiltonian decides which factorizations are candidates; the state decides which candidate is physical; the model cost decides whether either is worth the transformation. In the limit where $H$ is exactly few-body in exactly one factorization the candidate set is a singleton and the state's role reduces to distinguishing a low-entanglement from a high-entanglement phase within a fixed split (the thermal control of Section 2.4 is this limit). The soft cost is what makes the candidate set a cloud in which the state's vote is real. We say this rather than hide it.

The principle has no power-law bias toward one-dimensional factorizations — a tensor-network listing is extensive in $n$ in every dimension, with $d$ entering through the coordination number — but a path has an unusually short adjacency list and at $M\sim24$ that mild compressibility can dominate. Section 5.5 treats a path as a live outcome for this reason. Dimension is not selected by the principle and is not claimed to be.

### 4.6 The conjecture

**Conjecture.** *For $H$ of the form (2.1) with expander couplings in some range of $(q,\kappa)$, and for ground-space states, there exists $F\in\mathcal{F}$ at moderate $(s,D)$ such that (i) $\Delta\mathcal{C}$ is a substantial fraction of $\mathcal{C}(F_0)$, (ii) the weight-carrying support hypergraph $\mathcal{S}_F$ has polynomial ball growth with exponent $d_s>1$ rather than expansion, and (iii) for two ground-space or first-excited states of the same $H$ the minimizers differ reproducibly in $\mathcal{S}_{F^*}$.*

This is the paper's only claim beyond definitions, and Section 5 is its test.
...
### 5.1 Spectral pre-test, as an empirical question
...
and exhaustive enumeration is possible only where the arithmetic allows it. At $m=1$ the number of partitions of $M$ modes into pairs is $(M-1)!!$: $10{,}395$ at $M=12$, $135{,}135$ at $M=14$, $2.0\times10^{6}$ at $M=16$, $6.5\times10^{8}$ at $M=20$, and $3.2\times10^{11}$ at $M=24$. Since each evaluation of $\mathcal{C}$ rewrites $H$ and compresses a tensor network, exhaustive certification of $\mathcal{F}_0$ is confined to $M\le14$ with $s_0=0$ and $V=\mathbb{1}$, where every pairing is evaluated. Within that class and at those sizes the conjecture is *falsified* or *confirmed*, not merely unsupported. At $M\in\{20,22,24\}$, $\mathcal{F}_0$ is sampled — a declared number of pairings drawn uniformly, with the fraction of the class covered reported — and all larger classes are searched by a declared ansatz from declared initializations, whose output is a local minimum. The honest statement for everything beyond exhaustible $\mathcal{F}_0$ is that the conjecture is disfavored if increasingly expressive searches systematically fail to find $\Delta\mathcal{C}>0$ with geometric support.

### 5.4 Three questions, with controls

1. **Compression.** Is there $F\in\mathcal{F}_k$ with $\mathcal{C}(F)$ substantially below $\mathcal{C}(F_0)$, with the honesty cut satisfied (most coupling weight of $\mathcal{S}_F$ on codebook hyperedges) and the state's bond dimension bounded rather than growing with $n$? *Controls:* thermal and Haar states should fail this; robustness under $p\to p\pm2$ and $\epsilon\to\epsilon/10$ is required.
2. **Geometry.** Does the weight-carrying $\mathcal{S}_{F}$ have polynomial ball growth with a stable exponent $d_s$, cut sizes scaling as $r^{d_s-1}$ for balls of radius $r$, and homology stable under the choice of which short cycles count as faces — or is it an expander, a path, or a complete graph with a backbone?
3. **State vote.** For the ground state and the first excited state of the same gapped $H$, do the minimizers differ in $\mathcal{S}_{F^*}$? *Controls:* the same state from several random initializations must return the same $\mathcal{S}_{F^*}$ up to site permutation before a difference between states is credited; prepared-state controls must give the same answer under $J_0\to J_0'$.
...
- **The pre-test finds no window,** or **no $F$ compresses** within exhaustible $\mathcal{F}_0$ at $M\le14$ and sampled or ansatz searches at $M\le24$ fail systematically. The conjecture is falsified within $\mathcal{F}_0$ at $M\le14$ and disfavored beyond it for the class (2.1) at accessible $M$.
- **Compression with a path or a backboned complete graph.** One dimension or mean field. A path found under a charged $O$ with no encoding is a fact about $H$, to be understood rather than read as a low-dimensional preview.
- **Compression with $d_s\in\{2,3\}$ that moves reproducibly under a change of state at fixed $H$.** The conjecture holds at small $M$ within the searched class, and finite-size scaling becomes the question.
...
**Claimed.** (a) The principle (1.1) over the class $\mathcal{F}$ is well-posed, static, non-vacuous, and honestly two-part. (b) The conjecture of Section 4.6. (c) The prediction that a non-geometric $H$ and a ground-space $\Psi$ can jointly select a finite-dimensional local tensor-product structure, and that the selection can change at fixed $H$.

**What would disfavor the conjecture.** A negative pre-test at accessible $M$; falsification within an exhaustible $\mathcal{F}_0$; systematic failure of increasingly expressive searches; or a proof that expander polynomials admit no factorization in $\mathcal{F}$ with $\Delta\mathcal{C}>0$ and geometric support.
...
*Model.* $L(O)=\log_2 s+s\big[\log_2\binom{M}{2}+p\big]$ for $O\in\mathcal{G}_s$; $L(\{B_i\})=\log_2\big[M!/((2m)!^{n}\,n!)\big]$; $L(V)=$ declared bits per gate times gate count. A general $O\in O(M)$ at precision $p$ would cost $\binom{M}{2}p$, which for $M=24$, $p=8$ is $2208$ bits — larger than $L(H\mid F_0)$ for a sparse $H$ with $\kappa=O(1)$, so a generic rotation cannot be paid for by the Hamiltonian term alone and must be paid for, if at all, by the state.

*Hamiltonian.* $L(E)$ for the codebook as an adjacency structure ($O(nc\log n)$ for a random hypergraph of degree $c$, less for a compressible one); per term, $\log_2|E(G_F)|+O(q\log 2m)+p$ on-codebook, $q\log_2 n+O(q\log 2m)+p$ off-codebook.

*State.* For a fermionic tensor network on the codebook with bond dimensions $\chi_e$: $\sum_e\log_2\chi_e$ plus $p\times$(tensor entries). For an area-law state on a codebook of bounded degree $c$ this is $O(n\,c\,\chi^{c}\,p)$, extensive in $n$ in every dimension.

*Ultraviolet presentation.* $\mathcal{C}(F_0)=L(\{B_i\}_{\mathrm{UV}})+L(E)+|E|(\log_2|E|+O(q)+p)+L(\Psi\mid F_0)$, the last term being whatever the ground state costs as a network on the expander $E$, which for a highly entangled state may be exponential in $M$.

## Appendix D — Provenance

This formulation was reached through objections from two critical readers, recorded because the repairs are part of the argument. *Round 1:* a dynamical selection principle (slowest entanglement growth) was found to consume the clock it should produce; replaced by a static two-term cost. *Round 2:* a hard few-body threshold makes the candidate set a CPR singleton; the cost was made soft. *Round 3:* a state cost conditional on $H$ is $O(1)$ for eigenstates (Appendix A); replaced by representation cost. *Round 4 (Version 1→1.1):* physical states had been defined by complexity "in some factorization," vacuous by Appendix B; $V$ was unrestricted in the principle and restricted in the experiment; the support hypergraph and the codebook shared a symbol. *Round 5 (1.1→1.2):* the state set still contained the forbidden phrase; thermal states had been called physical although they reduce the principle to spectrum-selection. *Round 6 (1.2→2):* the rotation $O$ was uncharged, so the principle transmitted the data but not the model and could reward encoding the answer in the basis — repaired by $L(F)$ and the finite class $\mathcal{G}_s$; the Fock vacuum was used without declaring the complex structure it requires — repaired by making $(H,J_0)$ the data for prepared states and using ground states for the primary claim; the mode count was inconsistent between definition and experiment — repaired by $M$; the sparse-SYK citation was misdescribed — corrected, and the pre-test reframed as an empirical question; a local optimizer's failure had been called a falsification — repaired by nested classes with an exhaustible $\mathcal{F}_0$; gauge redundancy had been claimed to follow from factorization — demoted to a target requiring a code subspace; a section claiming consequences of finite Hilbert-space dimension was false and was removed; a list of predictions that did not follow from the principle was removed; and the predecessor [6] was added. *Round 7 (2→2.1):* the claim that $\mathcal{F}_0$ is exhaustively enumerable at $M=20$ was arithmetically wrong — there are $(M-1)!!\approx6.5\times10^{8}$ pairings — and certification was confined to $M\le14$, with sampling and a declared coverage fraction at larger $M$. We do not claim the list is closed.

## Appendix E — Motivating Bets (Not Claims of This Paper)

The proposal was developed inside a broader picture that we record for honesty and disown as results: that locality and Lorentz invariance are emergent rather than protected, with the Bombelli–Henson–Sorkin fork [23] taken on the nonlocal side; that the metric and gauge fields are not independently specified ultraviolet fields; that time is relational and no physical global clock exists; that interior operators of horizons are state-dependent while laboratory operators are not; that the cosmological constant has a floor set by a finite Hilbert space with no derived time scale for its variation. None of these follows from Sections 2–5. Several were stated as commitments in earlier versions and some were stated wrongly. They explain why the authors found the conjecture worth testing; they are not reasons to believe it.
...
[6] V. Shokrian Zini, A. R. Brown, M. H. Freedman, "The smallest interacting universe," *JHEP* **01**, 082 (2023). See also M. H. Freedman, V. Shokrian Zini, "The universe from a single particle," *JHEP* **01**, 140 (2021).
